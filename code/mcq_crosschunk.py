...
def append_mcq_to_file(mcq, filename="mcq_database2.jsonl"):
...
def build_cross_chunk_context(
    ...
    source_files = {os.path.basename(query_chunk.metadata["source"])}

    for idx, _ in top_k_chunks:
        chunk = all_chunks[idx]
        texts.append(chunk.page_content)
        source_files.add(os.path.basename(chunk.metadata["source"]))

    combined = "\n\n".join(texts)
    if len(combined) > max_chars:
        combined = combined[:max_chars]

    return combined, list(source_files)
...
def process_chunk_parallel(idx, chunk_doc):
    ...
    top_k_chunks = get_top_k_similar_chunks(
        query_idx=idx - 1,   # idx is 1-based in executor
    ...
    generated_mcqs = generate_mcq(combined_text)
    if generated_mcqs:
        for mcq in generated_mcqs:
            ...
            mcq["cross_chunk_sources"] = source_files
            mcq["combined_context"] = combined_text
...
if __name__ == "__main__":
    ...
    ALL_CHUNK_EMBEDDINGS = build_all_chunk_embeddings(text_chunks)
    ...
    if MODE == "generate_only":
        ...
        if not os.path.exists("mcq_database2.json"):
            print(" 'mcq_database2.json' not found.")
        else:
            with open("mcq_database2.json", "r") as f:
    ...
    if final_mcq_database:
        with open("mcq_database2.json", "w") as f:
            json.dump(final_mcq_database, f, indent=2)
        print(" MCQs saved to 'mcq_database2.json'")
