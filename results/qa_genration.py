...
"""
    ...
    for qa_pair in qa_pairs:
        ...
        for r in range(REFINEMENT_ROUNDS):
            ...
            critique_text = critique_qa(chunk, topics, test_points, qa_pair, qa_feedback)
            last_critique = critique_text
            
            refined = refine_qa_once(chunk, topics, test_points, qa_pair, qa_feedback, critique_text)
            if refined:
                qa_pair = refined
                qa_feedback = qa_answer_own_question(qa_pair)
            else:
                print("  ⚠️ Refinement failed, using current version")
                break
        
        # Add metadata
        # qa_pair["topics"] = topics
        # qa_pair["test_points"] = test_points
        # qa_pair["qa_feedback"] = qa_feedback
        # qa_pair["last_critique"] = last_critique
...
def process_chunk_parallel(idx, chunk_doc, filename):
    ...
    # Generate both types of Q/A with SRefine
    normal_qa = generate_normal_qa(chunk_text)
    reasoning_qa = generate_reasoning_qa(chunk_text)

    print(f"✅ Normal Q/A: {len(normal_qa)} pairs")
    print(f"✅ Reasoning Q/A: {len(reasoning_qa)} pairs")

    if normal_qa or reasoning_qa:
        qa_entry = {
            # "file": filename,
            # "chunk_id": idx,
            "normal_qa": normal_qa,
